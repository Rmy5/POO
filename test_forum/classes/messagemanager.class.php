...
class MessageManager{
	...
	public function getAllMessages($id, $sort){
		...
		$count = SPDO::getInst()->MakeSelect('SELECT COUNT(c_id) as test FROM conversation WHERE c_id = :id', array(':id'=>$id));
	...
	public function getCountMessages($id){
		...
		$result = SPDO::getInst()->MakeSelect('SELECT COUNT(m_id) as count FROM message m INNER JOIN conversation c ON m_conversation_fk = c.c_id WHERE c.c_id = :id', array(':id'=> $id));
	...
	public function addMessage($params = array()){
		...
		// Ajout d'un message
		$add = SPDO::getInst()->MakeInsert('INSERT INTO message(m_contenu, m_date, m_auteur_fk, m_conversation_fk) VALUES (:message, NOW(), :id, :conversation)', array(':message'=>$message, ':id'=>$id, ':conversation'=>$conversation));
		return $add;
	}
}
