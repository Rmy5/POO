...
<html lang="en">
	...
	<body>
		...
		if (isset($_POST['flash'])) {
			...
			// VERIF DES TICKETS CHOISIS - MAJ PORTE-MONNAIE et SESSION TICKETS - AFFICHAGE DES TICKETS
			if (!isset($_SESSION['tickets'])) $_SESSION['tickets'] = 0;
			if (isset($_POST['sub']) && isset($_POST['check_list'])) {
				...
				else echo 'Vous n\'avez pas assez d\'argent';
			}
			elseif (isset($_POST['sub']) && !isset($_POST['check_list'])) echo 'Choisissez au moins un ticket';

			elseif (isset($_SESSION['tickets']) && $_SESSION['tickets'] != 0) {
			 	echo '<div id="tickets">Nombre de tickets : '.count($_SESSION['tickets']).'<br><strong>';
				foreach ($_SESSION['tickets'] as $ticket) echo $ticket. ' ';
				echo '</strong></div>';
			}
			elseif (!isset($_POST['sub']) && $_SESSION['tickets'] == 0) echo '<div id="tickets">Nombre de tickets : 0</div>';
			...
			// 		foreach ($_SESSION['tickets'] as $ticket) echo $ticket. ' ';
			// 		echo '</strong></div>';
			// 		echo '<form action="" method="POST"><input type="submit" name="tirage" value="Lancer la tombola" id="lancer" class="in1"></form>';
			// 	}
	...
	?>
		...
		<?php
			// AFFICHAGE GRILLE TICKETS
			if (isset($_POST['grille']) && !isset($_POST['sub'])) {
				echo '<br><br><br><br><form id="grille" action="" method="POST">';
				for ($i=1; $i <=100 ; $i++) { 
			...
			// AFFICHAGE BOUTON LANCEMENT - grille
				if (isset($_POST['sub']) && $_SESSION['tickets'] != 0) {
			...
			// TIRAGE	
		if (isset($_POST['tirage']) && isset($_SESSION['tickets']) && $_SESSION['tickets'] != 0 && !isset($_SESSION['tirage'])) {
				$tirage = flash(3);
			...
			elseif (isset($_POST['tirage']) && !isset($_SESSION['tickets'])) echo 'Vous n\'avez pas de tickets';
			...
			if (isset($_SESSION['result'])){
				...
				echo '<br><a href="tombola3.php">Jouer encore ?</a>';
			}

			if (isset($_POST['tirage']) && !isset($_SESSION['result']) && isset($_SESSION['tickets']) && $_SESSION['tickets'] != 0){
				if ($_SESSION['porte_monnaie'] == 0) {
			 		session_destroy();
			 		echo '<br><br>Vous n\'avez plus d\'argent<br><br><a href="tombola3.php">Nouvelle partie ?</a>';
				}
				else{
